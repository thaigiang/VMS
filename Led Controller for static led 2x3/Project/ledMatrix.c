...
void convertData(u8* ledData)
{
	...
	{
		for(i = 0; i < hwConfig.ledHeight/48; i ++) // Convert for each cabin row
		{
			u16 startIndex = i*hwConfig.ledWidth*48/4;// 0x6C0 is data for one row cabin 144x48(pixel)/4(pixel per byte) = 1728 = 0x6C0
			for (idx = 0; idx < hwConfig.ledWidth/4; idx ++)
			{
				for(index = 0; index < 4; index ++)
				{
					...
					{
						dataTemp = 0;
						for(n = 0; n < 12 ; n ++) // there are 6 module line per cabin, and each module divided into 2 separate half 
						{
							dataTemp |= ((ledData[startIndex + idx + index * hwConfig.ledWidth / 4 + n * hwConfig.ledWidth] >> (2 * k)) & 0x03) << (n * 2);
						}
						dplDataTemp[index_count++] = dataTemp&0xFFFF;
						dplDataTemp[index_count++] = (dataTemp>>16)&0xFFFF;
						
						//re-calculate value of data fit for hardware error, delete if hardware is fixed
//						tempValue = dplDataTemp[index_count-1] & 0x81;				 // bit 1, 8
//						tempValue |= (dplDataTemp[index_count-1] & 0x40) >> 1; // bit 7 -> 6
//						tempValue |= (dplDataTemp[index_count-1] & 0x20) >> 2; // bit 6 -> 4
//						tempValue |= (dplDataTemp[index_count-1] & 0x10) >> 3; // bit 5 -> 2
//						tempValue |= (dplDataTemp[index_count-1] & 0x08) << 3; // bit 4 -> 7
//						tempValue |= (dplDataTemp[index_count-1] & 0x04) << 2; // bit 3 -> 5
//						tempValue |= (dplDataTemp[index_count-1] & 0x02) << 1; // bit 2 -> 3
//						dplDataTemp[index_count-1] = tempValue;
						//end re-calculate
					}
				}
			}
...
void OE_Control(bool oeState)
{
	...
	else if(hwConfig.ledType == SCAN_STATIC)
	{
		if(!oeState)
		{
			LED_OE_PORT->BSRR = LED_OE_PIN;
		}
		else
		{
			LED_OE_PORT->BRR = LED_OE_PIN;
		}	
	}
}
...
void display(u8* data)
{
	...
	else if(hwConfig.ledType == SCAN_STATIC)
	{
		...
		count_temp = 0;
		for(index = 0; index< 4*hwConfig.ledWidth; index ++)
		{
			GPIOC->ODR = ~dplDataTemp[count_temp++];			
			GPIOE->ODR = ~dplDataTemp[count_temp++];
			delays(2);
			// Latch clock
			LED_CLK_PORT->BRR = LED_CLK_PIN1;
			delays(2);
			LED_CLK_PORT->BSRR = LED_CLK_PIN1;
		}
		
		//delays(5);
		LED_LAT_PORT->BRR = LED_LAT_PIN1;
		delays(5);
		LED_LAT_PORT->BSRR = LED_LAT_PIN1;
		delays(5);
		LED_LAT_PORT->BRR = LED_LAT_PIN1;
		delays(5);
		...
		for(index = 0; index< 4*hwConfig.ledWidth; index ++)
		{
			...
			delays(2);
			// Latch clock
			LED_CLK_PORT->BRR = LED_CLK_PIN2; 
			delays(2);
			LED_CLK_PORT->BSRR = LED_CLK_PIN2;
		}
		
		
		LED_LAT_PORT->BRR = LED_LAT_PIN2;
		delays(5);
		LED_LAT_PORT->BSRR = LED_LAT_PIN2;
		delays(5);
		LED_LAT_PORT->BRR = LED_LAT_PIN2;
		delays(5);
...
//			GPIOE->ODR = dplDataTemp[count_temp++];
//			// Latch clock
//			LED_CLK_PORT->BRR = LED_CLK_PIN3;  
//			LED_CLK_PORT->BSRR = LED_CLK_PIN3;
//		}
//		
//		LED_LAT_PORT->BRR = LED_LAT_PIN3;
//		LED_LAT_PORT->BSRR = LED_LAT_PIN3;
