...
#define DMA1_Channel5_EN			(0x42400B00)
#define DMA2_Channel2_EN			(0x42408380)
#define DMA1_Channel5_TCIE			(0x42400B04)
#define DMA2_Channel2_TCIE			(0x42408384)


#define VTOTAL	53							/* Total bytes to send through SPI */
//__align(4) u8 fb[VID_VSIZE][VID_HSIZE+2];	/* Frame buffer */
__align(4) u8 fbR[VID_VSIZE][VID_HSIZE+3];	/* Frame buffer */
__align(4) u8 fbG[VID_VSIZE][VID_HSIZE+3];	/* Frame buffer */
...
void TIMER_Configuration(void)
{
	...
	*/
	...
	Channel2Pulse = 8; 		/* HSYNC + BACK PORCH */
...
/*
	...
	DMA_InitStructure.DMA_MemoryBaseAddr = (u32) &fbR[0][0];
	...
	DMA1_Channel3->CMAR = (u32) &fbR[0][0];
...
// 	Init SPI2	
	...
	DMA_InitStructure.DMA_MemoryBaseAddr = (u32) &fbG[0][0];
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralDST;
	DMA_InitStructure.DMA_BufferSize = VTOTAL;
	DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
	...
	DMA1_Channel5->CCR &= ~1;
	DMA1_Channel5->CNDTR = VTOTAL;
	DMA1_Channel5->CMAR = (u32) &fbG[0][0];
	...
	DMA2_Channel2->CCR &= ~1;
	DMA2_Channel2->CNDTR = VTOTAL;
	DMA2_Channel2->CMAR = (u32) &fbR[0][0];
...
__irq void TIM3_IRQHandler(void)
{
	if (vflag)
	{				
		DMA2_Channel2->CCR = DMA1_Channel5->CCR  = 0x93 ;
	}
	TIM3->SR = 0xFFFB; //~TIM_IT_CC2;
}
...
{	
	...
	if(intCount == 2)
	{
		...
		if (vdraw == 4)
		{
			...
			{				
				DMA1_Channel5->CMAR += VTOTAL;
				DMA2_Channel2->CMAR += VTOTAL;
			}
		}
	}
}
