...
extern u8			fbR[VID_VSIZE][VID_HSIZE+3];
extern u8			fbG[VID_VSIZE][VID_HSIZE+3];
...
//	return			none
//*****************************************************************************
void gdiBitBlt(PGDI_RECT prc, i16 x, i16 y, i16 w, i16 h, pu8 bm, u16 rop,COLOR color) {
...
//	Get offset to frame buffer in bit-banding mode
		...
		if(color == RED_COLOR)
			k = (u32) (&fbR - 0x20000000);
		else if(color == GREEN_COLOR)
			k = (u32) (&fbG - 0x20000000);			
		k += offs;
...
//	return:			none
//*****************************************************************************
void gdiPoint(u16 x, u16 y, u16 rop,COLOR color) {
...
//	return			none
//*****************************************************************************
void gdiLine(PGDI_RECT prc, i16 x1, i16 y1, i16 x2, i16 y2, u16 rop,COLOR color) {
...
//	return			none
//*****************************************************************************
void	gdiRectangle(i16 x0, i16 y0, i16 x1, i16 y1, u16 rop,COLOR color) 
{
	gdiLine(NULL,x0,y0,x1,y0,rop,color);
	gdiLine(NULL,x0,y1,x1,y1,rop,color);
	gdiLine(NULL,x0,y0,x0,y1,rop,color);
	gdiLine(NULL,x1,y0,x1,y1,rop,color);
}
//*****************************************************************************
...
//	return			none
//*****************************************************************************
void gdiDrawText(PGDI_RECT prc, pu8 ptext, u16 style, u16 rop,COLOR color) 
{
	u16		l, i, pos, xp;
	u8		c;
	pu8		ptx;
	...
	for (i = 0; i < l; i++) {
		...
		if (c >= GDI_SYSFONT_OFFSET) {
			...
			gdiBitBlt(prc, xp, 0, GDI_SYSFONT_WIDTH, GDI_SYSFONT_HEIGHT, ptx, rop,color);
...
//	return			none
//*****************************************************************************
void gdiDrawTextEx(i16 x, i16 y, pu8 ptext, u16 rop,COLOR color) {
...
pu8		ptx;
	...
	for (i = 0; i < l; i++) {
		...
		if (c >= GDI_SYSFONT_OFFSET) {
			pos = (u16) (c - GDI_SYSFONT_OFFSET) * GDI_SYSFONT_BYTEWIDTH * GDI_SYSFONT_HEIGHT;
			ptx = ((pu8) gdiSystemFont) + pos;
			gdiBitBlt(NULL, xp, y, GDI_SYSFONT_WIDTH, GDI_SYSFONT_HEIGHT, ptx, rop,color);
			xp += GDI_SYSFONT_WIDTH;
