...
#define LOCAL_PORT_END_NUMBER   (10000u)
...
extern struct BSDSocket  BSDSocketArray[BSD_SOCKET_COUNT];
/****************************************************************************
  Section:
	TCB Definitions
  ***************************************************************************/
...
void TCPDisconnect(TCP_SOCKET hTCP)
{
	...
	switch(MyTCBStub.smState)
	{
		...
		case TCP_ESTABLISHED:
			// Check if socket is berkeley socket
			if((hTCP == BSDSocketArray[0].SocketID)||(hTCP == BSDSocketArray[1].SocketID))
			{
				CloseSocket();
				break;
			}
