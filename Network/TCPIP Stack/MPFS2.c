...
static volatile MPFS_STUB MPFSStubs[MAX_MPFS_HANDLES+1];

// Allows the MPFS to be locked, preventing access during updates
static volatile BOOL isMPFSLocked;

// FAT record cache
static volatile MPFS_FAT_RECORD fatCache;

// ID of currently loaded fatCache
static volatile WORD fatCacheID;

// Number of files in this MPFS image
static volatile WORD numFiles;
...
/****************************************************************************
  ...
  ***************************************************************************/
	...
	// An address where MPFS data starts in program memory.
  		...
  		extern ROM far BYTE MPFS_Start[];
