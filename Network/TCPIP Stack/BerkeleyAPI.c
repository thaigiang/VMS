...
int connect( SOCKET s, struct sockaddr* name, int namelen )
{
	...
	else
	{
		...
		if(socket->bsdState == SKT_CREATED)
		{
	        ...
	        if(gAutoPortNumber >10000u) // reset the port numbers
				gAutoPortNumber = 1024;
...
//	socket = &BSDSocketArray[s];
	...
	if(BSDSocketArray[s].bsdState == SKT_CLOSED)
		return SOCKET_ERROR;

	if(BSDSocketArray[s].SocketType == SOCK_DGRAM) //UDP
	{
		// Decide the destination IP address and port
		remoteInfo.IPAddr.Val = BSDSocketArray[s].remoteIP;
		wRemotePort = BSDSocketArray[s].remotePort;
		if(to)
		{
			...
			if(BSDSocketArray[s].bsdState == SKT_CREATED)
			{
		...
		if(UDPSocketInfo[BSDSocketArray[s].SocketID].remoteNode.IPAddr.Val != remoteInfo.IPAddr.Val)
		{
			if(ARPIsResolved(&remoteInfo.IPAddr, &remoteInfo.MACAddr))
			{
				memcpy((void*)&UDPSocketInfo[BSDSocketArray[s].SocketID].remoteNode, (void*)&remoteInfo, sizeof(remoteInfo));
			}
			else
			{
		...
		if(UDPIsPutReady(BSDSocketArray[s].SocketID))
		{
			// Set the proper remote port
			UDPSocketInfo[BSDSocketArray[s].SocketID].remotePort = wRemotePort;
	...
	else if(BSDSocketArray[s].SocketType == SOCK_STREAM) //TCP will only send to the already established socket.
	{
		if(BSDSocketArray[s].bsdState != SKT_EST)
			return SOCKET_ERROR;
		...
		size = TCPPutArray(BSDSocketArray[s].SocketID, (BYTE*)buf, len);
		if(size)
			return size;
	}
	return SOCKET_ERROR;
