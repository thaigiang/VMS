...
//0x1D7D8 = 0x1D7C0 + 0x19 - 0x01(start addr + length of HW_CONFIG and WN_CONFIG - 0x01 byte flag)
#define CONFIG_ADDRESS		 0x1D7D8 
...
#define IP_ADDRESS3			110

#define HOST_IP_ADDRESS0	255
#define HOST_IP_ADDRESS1	255
#define HOST_IP_ADDRESS2	255
#define HOST_IP_ADDRESS3	255
...
#define BOOTLOADER_TIMEOUT	(10ul*SECOND)
...
BYTE flashReadByte(unsigned long addr);
void m_memcpy(unsigned char* dest,unsigned char* src,unsigned int length);
...
// include 4 bytes IP address, 6 bytes MAC address and 1 byte for check flag
// static BYTE 		config[11]; 

#pragma udata NETWORKDATA
static struct
{
	BYTE			vMAC[6];
	DWORD_VAL		dwvIP;
...
void main(void)
{
	...
	static unsigned long addr = CONFIG_ADDRESS;
	static signed int check = 0x00;
	static BOOL fSecure = TRUE;
	...
	if(RCONbits.NOT_RI || (wvChecksum.Val != CalcIPChecksum((BYTE*)&MyAddress, 6+4)))
	{
		...
		/*dwvHostIP.v[0] = HOST_IP_ADDRESS0;
		dwvHostIP.v[1] = HOST_IP_ADDRESS1;
		dwvHostIP.v[2] = HOST_IP_ADDRESS2;
		dwvHostIP.v[3] = HOST_IP_ADDRESS3;*/


		check = flashReadByte(addr++);
		if(check == 0x60) //check flag for saved IP address
		{
			MyAddress.vMAC[0] = flashReadByte(addr++);
			MyAddress.vMAC[1] = flashReadByte(addr++); 
			MyAddress.vMAC[2] = flashReadByte(addr++);
			MyAddress.vMAC[3] = flashReadByte(addr++);
			MyAddress.vMAC[4] = flashReadByte(addr++);
			MyAddress.vMAC[5] = flashReadByte(addr++);
			MyAddress.dwvIP.v[0] = flashReadByte(addr++);
			MyAddress.dwvIP.v[1] = flashReadByte(addr++);
			MyAddress.dwvIP.v[2] = flashReadByte(addr++);
			MyAddress.dwvIP.v[3] = flashReadByte(addr++);
			/*dwvHostIP.v[0] = flashReadByte(addr++);
			dwvHostIP.v[1] = flashReadByte(addr++);
			dwvHostIP.v[2] = flashReadByte(addr++);
			dwvHostIP.v[3] = flashReadByte(addr++);*/
		}	
	}	
	...
	while(1)
	{
		...
		case 0x0008:		// 0x0800 is IP type, but this is big endian
			...
			/*if(check == 0x60)
			{
				DWORD_VAL SourceIP = IPHeaderPtr->SourceIP;
				if((SourceIP.v[0]!=dwvHostIP.v[0])||
					(SourceIP.v[1]!=dwvHostIP.v[1])||
					(SourceIP.v[2]!=dwvHostIP.v[2])||
					(SourceIP.v[3]!=dwvHostIP.v[3])
					)
				{
					//EPKTCNT = 1u;
					//break;
